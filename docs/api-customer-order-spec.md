...
- [SF-46: Fix Customer Order Validation and Payment Handoff Gaps](https://tuanwork.atlassian.net/browse/SF-46)
...
Giá và amount là số nguyên theo đơn vị VND. Các field khai báo integer không
chấp nhận số có phần lẻ hoặc số được gửi dưới dạng JSON string.
...
- `totalAmount` tối đa là `9,999,999,999` VND để khớp `NUMERIC(12,2)` và quy tắc whole-VND.
- `paymentMethod` trong MVP chỉ nhận `CARD`; `COD` bị từ chối vì COD workflow
  chưa thuộc baseline.
...
| `paymentMethod` | string enum | yes | Chỉ `CARD` → `orders.payment_method`; `COD` bị từ chối trong MVP. |
| `items` | array | yes | Ít nhất 1 phần tử |
| `items[].productId` | number (long) | yes | JSON integer; không nhận fraction/string; `products.id`; mỗi `productId` chỉ xuất hiện **một lần** trong `items` |
| `items[].quantity` | number (int) | yes | JSON integer `> 0`; không nhận fraction/string |
...
| `paymentMethod` | string enum | `orders.payment_method` | Luôn là `CARD` khi create thành công |
| `totalAmount` | number | `orders.total_amount` | NUMERIC(12,2); số nguyên VND; = sum line totals; tối đa `9,999,999,999` |
...
| `items[].unitPrice` | number | `order_items.unit_price` | Snapshot số nguyên VND từ `products.price` |
| `items[].quantity` | number (int) | `order_items.quantity` | |
| `items[].lineTotal` | number | computed | `unitPrice * quantity` theo VND (không lưu cột riêng) |
...
Error shape thống nhất với catalog (`message` + `status`). Order API luôn trả các
collection chi tiết; collection không áp dụng cho lỗi hiện tại có giá trị rỗng.
...
|---|---|---|
| Missing/blank/too-long field | `Invalid order request` | `fieldErrors: { "field.path": "reason" }` |
| Fraction/string cho `productId` hoặc `quantity` | `Malformed request body` | Không coercion/truncation |
...
| `paymentMethod` không phải `CARD` (bao gồm `COD`) | `Invalid payment method` | |
...
| `totalAmount` vượt giới hạn DB | `Order total exceeds supported maximum` | Không ghi order/reservation/movement |
...
| `fieldErrors` | Map từ JSON field path sang lý do validation; rỗng với business error/malformed body |
...
| `payment_method` | VARCHAR(20) | `CARD` trong MVP; schema còn giữ `COD` cho mở rộng |
| `total_amount` | NUMERIC(12,2) | Computed sum theo VND nguyên |
...
| `unit_price` | NUMERIC(12,2) | Snapshot số nguyên VND từ `products.price` |
...
- Map `fieldErrors` vào đúng input và giữ nguyên dữ liệu/form khi request bị từ chối
- Sau 201: chuyển sang payment-simulation handoff hiển thị chính xác `order.id`, `status`
  và `totalAmount`; chưa gọi payment API (SF-4)
...
| 13 | `paymentMethod=COD` hoặc giá trị invalid | 400; không tạo order/reservation/movement |
...
| 16 | Fraction/string cho `productId` hoặc `quantity` | 400 malformed; không coercion; không tạo order/reservation/movement |
| 17 | Customer/shipping field invalid | 400; `fieldErrors` chỉ rõ field path; form giữ dữ liệu hợp lệ |
| 18 | Total vượt `9,999,999,999` VND | 400 trước persistence; boundary `9,999,999,999` vẫn 201 |
| 19 | Create thành công trên frontend | Payment handoff hiển thị đúng id/status/total; không gọi payment API |
